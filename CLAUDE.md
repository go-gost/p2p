...
| `--link` | *(empty)* | device link `"device=peer-key"` (`"p2p0=<key>"` or `"tap:veth0=<key>"`); DERP mode, Linux, exclusive with `--target`/`--forward`, single link |
...
`forwards` list are additive (same for `--link` / `links`), so a config can fully replace
the command line.
...
**Device link** (`--link "device=peer-key"`, [dev.go](dev.go) / [dev_linux.go](dev_linux.go)): bridges a pre-existing local tun/tap to the peer's same-kind device over one persistent stream — a minimal point-to-point link (groundwork for tun-to-tun). The p2p host **neither creates nor configures** the device: addresses, routes, MTU and admin-up are the caller's job, and the device must be free, because a tun is exclusive-open and so cannot be shared with a gost tun listener. It only opens it (Linux `/dev/net/tun` + `TUNSETIFF` with `IFF_NO_PI`; other platforms are a stub). The device is treated as an opaque chunk device: each `Read` becomes one 2-byte-length-prefixed frame, which preserves packet boundaries for tun/tap and the byte stream for pipe/socket devices — the two ends must be the same kind, since a stream source into a packet sink would scramble boundaries. A single long-lived `devReadLoop` reads the device and writes the current link stream (single goroutine because a device `Read` cannot be interrupted — tun has no deadline; chunks read while the link is down are dropped). The opener (smaller public key, the same rule as the mux session) keeps the stream alive via `linkLoop`; the responder is served by the existing `acceptLoop`, which in device mode routes the inbound stream to the device instead of `--target`. Device mode is mutually exclusive with `--target` and supports a single link; the data path is unencrypted like the rest of the data plane.
