...
- `OpenTunnel(peer, network)` — opens a local endpoint on `--bind` and replies `{ok, id, endpoint}`. `network` is `tcp` (a byte stream, the default) or `udp` (a datagram endpoint); anything else is `codes.InvalidArgument`. In stub mode `peer` must be `host:port` (validated) and only `tcp` is accepted (a udp channel needs a peer key); in DERP mode `peer` must be a base64 32-byte public key. Business failures (listen error) answer `ok:false` with gRPC status OK; clients treat either channel as failure.
