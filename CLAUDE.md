...
- `Status` — tunnel count (pending records included) plus transport stats: gauges `direct_peers`/`derp_peers` (where each peer's traffic goes now), cumulative counters `punch_attempts`/`punch_success`/`streams_direct`/`streams_derp`, and `PeerTransports` — the same classification per base64 peer key, for a caller that lists peers (in-process only: the proto has no field for it, so the gRPC transport carries the aggregate gauges). The gauges probe each `directConn` with the side-effect-free `live()` — never `session()`, which tears a dead session down and schedules a re-punch, so a status query would churn connections. gRPC reflection is registered, so `grpcurl -plaintext <addr> proto.P2P/Status` works without shipping the proto (with `--token` set, add `-H 'token: …'`).
